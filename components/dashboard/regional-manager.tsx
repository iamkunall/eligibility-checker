...
import { Sprout, PlusCircle, BookOpen, Users, Settings, type LucideIcon } from "lucide-react"
...
const DashboardCard = ({ card }: { card: DashboardCard }) => {
  ...
  return (
    ...
    >
      <card.icon className={`h-8 w-8 text-asset-dark mb-2 transition-transform ${isHovered ? "scale-110" : ""}`} />
      <h2 className="text-xl font-semibold text-asset-dark">{card.title}</h2>
      {card.description && <p className="text-sm text-asset-medium mt-1">{card.description}</p>}
    </Link>
  )
}
